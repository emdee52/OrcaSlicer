...
def get_orcamcp_executable() -> str | None:
    ...
    if system == "Darwin":  # macOS
        ...
        # Dev build paths (relative to project root)
        paths.extend([
            os.path.join(project_root, "build", "src", "Release", "orca-slicer.exe"),
            os.path.join(project_root, "build", "OrcaSlicer", "orca-slicer.exe"),
            os.path.join(project_root, "build", "Release", "orca-slicer.exe"),
        ])
        # Installation paths
        paths.extend([
            os.path.join(os.environ.get("ProgramFiles", "C:\\Program Files"), "OrcaSlicer", "orca-slicer.exe"),
            os.path.join(os.environ.get("ProgramFiles(x86)", "C:\\Program Files (x86)"), "OrcaSlicer", "orca-slicer.exe"),
...
def launch_orcamcp() -> dict:
    ...
    try:
        ...
        # Opt in to the MCP endpoint in the launched app.
        launch_env = os.environ.copy()
        launch_env["ORCA_EXT_MCP"] = "1"
