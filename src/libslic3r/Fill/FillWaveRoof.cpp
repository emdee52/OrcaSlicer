...
// [ORCAPORT FILE] The fork's WAVEROOF_LOG (NeoDebug channel) is dropped: no NeoDebug port.
#define WAVEROOF_LOG(body) do {} while (0)
...
// [ORCAPORT:SU-4] target 2.5 stores extrusion polylines as 3D (Polyline3); this roof algorithm
// works in the 2D layer plane, so convert at the ExtrusionPath boundary and back for scoring.
static Points3 wave_to_points3(const Points &pts)
{
    Points3 out;
    out.reserve(pts.size());
    for (const Point &p : pts)
        out.emplace_back(Point3(coord_t(p.x()), coord_t(p.y()), coord_t(0)));
    return out;
}

static Polyline wave_to_polyline(const Polyline3 &pl)
{
    Polyline out;
...
ExtrusionPath make_wave_path(const Polyline &polyline, const Flow &flow, ExtrusionRole role)
{
    ...
    path.polyline.points = wave_to_points3(polyline.points);
    return path;
}
                        ...
                        ExtrusionRole    role)
{
    ...
    auto support_score = [&point_at_distance](const Polyline &candidate, const ExtrusionPaths &support_paths, coord_t support_reach, coord_t prefix_length) {
        ...
        for (auto it = support_paths.rbegin(); it != support_paths.rend(); ++it) {
            // [ORCAPORT:SU-4] 3D path -> 2D for the planar scoring.
            const Polyline support_2d = wave_to_polyline(it->polyline);
            if (support_2d.points.size() < 2)
                continue;

            double score = 0.;
            for (const auto &[distance_along, weight] : samples) {
                Point sample = point_at_distance(candidate, distance_along);
                std::pair<int, Point> foot = foot_pt(support_2d.points, sample);
                int seg_idx = foot.first;
                if (seg_idx < 0 || size_t(seg_idx + 1) >= support_2d.points.size())
                    continue;

                const Point &a = support_2d.points[size_t(seg_idx)];
                const Point &b = support_2d.points[size_t(seg_idx + 1)];
...
void append_zig_zag_front_levels(ExtrusionPaths &roof_region, const std::vector<Polylines> &front_levels, const Flow &wave_flow, coord_t connector_limit, ExtrusionRole role)
{
    ...
    auto append_or_start = [&](Polyline &&front) {
        ...
        if (d_flip < d_keep)
            front.reverse();
        // [ORCAPORT:SU-4] current is an ExtrusionPath (3D polyline); front is a 2D Polyline.
        const Points3 front3 = wave_to_points3(front.points);
        if (current.last_point() == front.first_point()) {
            for (size_t i = 1; i < front3.size(); ++i)
                current.polyline.append(front3[i]);
        } else {
            for (const Point3 &p : front3)
                current.polyline.append(p);
        }
    };
