// [ORCAPORT FILE] SupportZones SupportZoneProbe - zone render/coverage probe (SU-5).
// Source: NEOTKOCM_RELEASE_2_44.md; fork sha c1d9cc2890.
...
// [ORCAPORT:SU-5] the fork's ZONEPROBE_LOG (NeoDebug WAVESUPPORT channel) is dropped: no NeoDebug port.
#define ZONEPROBE_LOG(body) do {} while (0)
...
ZoneProbe probe_zone(const ModelObject &object, const ModelVolume &enforcer, float grid_step_mm)
{
    ...
    std::vector<igl::Hit<float>> hits_zone;
    std::vector<igl::Hit<float>> hits_parts;
    ...
    for (float y = bb_min.y() + 0.5f * step; y < bb_max.y(); y += step) {
        for (float x = bb_min.x() + 0.5f * step; x < bb_max.x(); x += step) {
            ...
            for (const igl::Hit<float> &hit : hits_parts) {
