...
void TreeSupport::detect_overhangs(bool check_support_necessity/* = false*/)
{
    ...
    // Clear and create Tree Support Layers. [ORCAPORT:PF-10-paint] When a second support pass
    // appends (painted tree regions after the classic pass), keep the existing layers.
    if (!m_object->support_pass_appends())
    ...
    for (const EnforcerBlockerType bs : m_object->painted_support_blockers())
        if (bs != EnforcerBlockerType::NONE && bs != EnforcerBlockerType::BLOCKER)
            m_object->project_and_append_custom_facets(false, bs, blockers);
