...
public:
    ...
    // [ORCAPORT:SU-8] Transition joint of this layer: 0 = none, 1 = interface-on-base,
    // 2 = object-on-interface, 3 = interface-on-object. Tagged at support-generation time and
    // consumed by the G-code generator to apply the transition-layer treatment.
    int                 transition_joint { 0 };
    // Footprint that actually touches the other material on this layer. Speed/flow are only
    // modified for paths inside it, so the rest of the layer is untouched. Empty = whole layer.
    ExPolygons          transition_area;
    // [ORCAPORT:SU-9] This support layer carries a woven base/interface interlock. Its base and
    // interface strips are printed at one speed (see GCode::_extrude).
    bool                support_weave { false };
    // [ORCAPORT:SU-10] This support layer holds the interface contact that touches the object:
    // top contact (support under the object) and/or bottom contact (support on top of an object).
    // Each can have its own speed/line-width override.
    bool                support_contact_top { false };
    bool                support_contact_bottom { false };
    ...
    // [ORCAPORT:PF-2] Smart counterbore bridging: per-layer (bridge material, fill angle) pairs
    // added by apply_counterbore_bridge_geometry(). Empty when the feature is off.
...
public:
    ...
    // [ORCAPORT:SU-1] read-only access to the printed tree areas for the cross-object support
    // occupancy builder (OrcaExt::neighbor_occupancy); the members themselves stay protected.
