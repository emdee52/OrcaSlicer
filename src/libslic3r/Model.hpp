...
public:
    ...
    bool                    is_counterbore_bridge_painted() const; // [ORCAPORT:PF-2]
...
public:
    ...
    // [ORCAPORT:PF-2] List of mesh facets painted for counterbore smart bridging.
    ...
    bool is_any_painted() const { return is_fdm_support_painted() || is_seam_painted() || is_mm_painted() || is_fuzzy_skin_painted() || is_counterbore_bridge_painted(); }
...
private:
    ...
    ModelVolume(ModelObject *object, const ModelVolume &other) :
        ...
        fuzzy_skin_facets(other.fuzzy_skin_facets), counterbore_bridge_facets(other.counterbore_bridge_facets), cut_info(other.cut_info), text_configuration(other.text_configuration), emboss_shape(other.emboss_shape)
    {
		assert(this->id().valid()); 
        assert(this->config.id().valid()); 
        assert(this->supported_facets.id().valid());
	...
	ModelVolume() : ObjectBase(-1), config(-1), supported_facets(-1), seam_facets(-1), mmu_segmentation_facets(-1), fuzzy_skin_facets(-1), counterbore_bridge_facets(-1), object(nullptr) {
		assert(this->id().invalid());
        assert(this->config.id().invalid());
	...
	template<class Archive> void load(Archive &ar) {
        ...
        mesh_changed |= t != counterbore_bridge_facets.timestamp();
...
extern bool model_counterbore_bridge_data_changed(const ModelObject &mo, const ModelObject &mo_new); // [ORCAPORT:PF-2]
