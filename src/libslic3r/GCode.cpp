    ...
    // Without this the dispatcher threw "Invalid argument supplied to extrude()".
        ...
        if (coll->no_sort) {
            for (const ExtrusionEntity* ee : coll->entities)
                gcode += this->extrude_entity(*ee, description, speed, region_perimeters, wipe_support);
        } else {
