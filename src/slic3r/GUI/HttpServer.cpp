...
{
    // [ORCAPORT:MCP-1] BEGIN - read the full Content-Length body, then process the request
    auto self(shared_from_this());
    ...
    // Some body bytes may already sit in the streambuf.
...
void session::read_next_line()
{
    ...
    async_read_until(socket, buff, '\r', [this, self](const boost::beast::error_code& e, std::size_t s) {
        if (!e) {
            ...
            if (line.length() == 0) {
                // [ORCAPORT:MCP-1] BEGIN - handle OPTIONS; route body-bearing requests through read_body/process_request
...
// [ORCAPORT:MCP-1] BEGIN - full-signature handler; legacy URL-only wrapper for existing OAuth callers
...
// [ORCAPORT:MCP-1] BEGIN - JSON response writer for the MCP endpoint
