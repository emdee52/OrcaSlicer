// [ORCAPORT FILE] GLGizmoAlignStack - Align & Stack gizmo (ported from Neotko's fork).
...
// one that moves) â€” an ordered chain of 3+ was never actually used, just
...
{
    // [ORCAPORT:AS-2] Always available. The order (#1 anchor, #2 mover) is built by clicking
    // objects in the scene, so it is usable with an empty selection.
    return true;
}
...
void GLGizmoAlignStack::data_changed(bool /*is_serializing*/)
{
    ...
    // restores via stable ids first (no dangling pointers), then re-tints â€”
...
// -----------------------------------------------------------------------------
// Order tracking â€” owned by the gizmo, selection is mirrored so that
...
// -----------------------------------------------------------------------------
// Scene highlight â€” tint each ordered object's volumes with its order color
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// Render â€” colored letter badges, no 3D handles
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// Mouse â€” click-to-order in the scene, plus face-pick mode
// -----------------------------------------------------------------------------
...
bool GLGizmoAlignStack::on_mouse(const wxMouseEvent& mouse_event)
{
    ...
    if (mouse_event.LeftDown() && !mouse_event.Dragging()) {
        ...
        if (hovered < 0)
            return false; // empty space â†’ camera keeps working as usual
        const GLVolumePtrs& volumes = m_parent.get_volumes().volumes;
...
// -----------------------------------------------------------------------------
// Face highlight â€” light up the triangle under the cursor (and the picked one)
...
// -----------------------------------------------------------------------------
// Transform application â€” #1 is always the anchor; #2 moves toward it.
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// Zone / ghost placement preview â€” AABB wireframes in the viewport for #1 and
...
void GLGizmoAlignStack::render_zone_and_ghosts()
{
    ...
    // use (compute_place_delta / compute_center_delta) â€” the preview cannot
    ...
    if (shader != nullptr) {
        ...
        if (show_ghosts) {
            ...
            ghost_col[3] = 0.35f;
            glsafe(::glLineWidth(3.6f)); // 3x the old 1.2f â€” was barely visible
    ...
    // ghost touches #1). Drawn straight into the foreground draw list â€” same
    ...
    // World â†’ ImGui screen point. False when the point is behind the camera or
