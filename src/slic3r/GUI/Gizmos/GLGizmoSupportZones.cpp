...
#include "GizmoStyle.hpp" // [ORCAPORT:SU-5] shared look for the fork's curve gizmos

#include "slic3r/GUI/GUI.hpp" // [ORCAPORT:SU-5] into_u8()
...
// [ORCAPORT:SU-5] NeoDebug is not ported; this local no-op shim keeps the fork's trace calls inert.
namespace Slic3r { namespace NeoDebug {
enum Channel { SUPPORTZONES = 0 };
inline bool enabled(Channel) { return false; }
inline bool is_enabled(Channel) { return false; }
inline void write(Channel, const std::string &) {}
inline std::string log_dir() { return std::string(); }
}} // namespace Slic3r::NeoDebug
...
{
    // [ORCAPORT:SU-5] ungated; needs one object selected (every zone is a ModelVolume of it).
...
void GLGizmoSupportZones::rebuild_zone_rows()
{
    ...
    for (int vi = 0; vi < int(mo->volumes.size()); ++ vi) {
        ...
        if (const ConfigOption *opt = v->config.option("support_zone_gesture"); opt != nullptr)
...
void GLGizmoSupportZones::unlock_selected_zone()
{
    ...
    mo->volumes[vi]->config.erase("support_zone_gesture");
    m_zones_dirty = true;
...
void GLGizmoSupportZones::duplicate_selected_zone()
{
    ...
    // clics, que es barato.
    dup->config.erase("support_zone_gesture");
                                            ...
                                            const Transform3d &inst, const ZoneGesture &g)
{
    ...
    v.config.set_key_value("support_zone_gesture", new ConfigOptionString(gesture_to_json(stamped)));
    ...
    // y el gesto no.
    v.config.set_key_value("support_zone_lean_deg", new ConfigOptionFloat(double(g.lean_deg)));
    ...
    v.config.set_key_value("support_zone_roof_only", new ConfigOptionBool(false));
    ...
    v.config.set_key_value("support_zone_solid", new ConfigOptionBool(false));
    ...
    v.config.set_key_value("support_zone_land_only", new ConfigOptionBool(false));
}
...
bool GLGizmoSupportZones::seed_support_defaults(ModelObject &mo)
{
    ...
    // [ORCAPORT:SU-4b] The fork seeded this here (SU-4 contact layer); the key is registered again
    // now that the contact layer is ported, so restore the seed: a zone-created object starts with
    // the contact wave off (matches the fork). See the earlier crash note in notes/SU-5.md.
...
void GLGizmoSupportZones::on_render_input_window(float x, float y, float /*bottom_limit*/)
{
    ...
    gizmo_push_window_style(); // 🚨 antes del Begin: WindowBg y el titulo se resuelven al abrir
    ...
    gizmo_pop_panel_style();

    GizmoImguiEnd();
    gizmo_pop_window_style();
}
...
inline float gizmo_u() { return ImGui::GetFontSize(); }
...
float gizmo_label_col()
{
    ...
    return std::min(w + 0.7f * gizmo_u(), ImGui::GetContentRegionAvail().x * 0.46f);
}
...
void dim_line_h(ImDrawList *dl, float x0, float x1, float y, ImU32 col, const char *label)
{
    ...
    if (label != nullptr && label[0] != '\0') {
        ...
        gizmo_draw_pill(dl, ImVec2((x0 + x1) * 0.5f - ts.x * 0.5f, y - ts.y - 6.f), label, col);
    }
}
...
void gizmo_section(const char *label)
{
    ImGui::Spacing();
...
bool gizmo_glyph_toggle(const char *id, float size, bool active, Glyph g, const char *tip)
{
    const bool pressed = ImGui::InvisibleButton(id, ImVec2(size, size));
    ImDrawList  *dl = ImGui::GetWindowDrawList();
    const ImVec2 a  = ImGui::GetItemRectMin(), b = ImGui::GetItemRectMax();
    const bool   hv = ImGui::IsItemHovered();
    dl->AddRectFilled(a, b, active ? gizmo_fade(GizmoCol::Accent, hv ? 1.0f : 0.85f)
                                   : gizmo_col_u32(hv ? GizmoCol::SurfaceHi : GizmoCol::Surface), 5.f);
    if (active)
        dl->AddRect(a, b, gizmo_col_u32(GizmoCol::AccentBright), 5.f, 0, 1.2f);
    draw_glyph(dl, ImVec2(a.x + size * 0.16f, a.y + size * 0.16f), size * 0.68f, g,
               active ? gizmo_col_u32(GizmoCol::Ink) : gizmo_fade(GizmoCol::TextDim, hv ? 1.0f : 0.85f));
    if (hv && tip != nullptr)
...
bool gizmo_glyph_button(const char *id, float size, Glyph g, bool danger, const char *tip,
                      GizmoCol tint = GizmoCol::TextDim, bool always_lit = false)
{
    const bool pressed = ImGui::InvisibleButton(id, ImVec2(size, size));
    ImDrawList  *dl = ImGui::GetWindowDrawList();
    const ImVec2 a  = ImGui::GetItemRectMin(), b = ImGui::GetItemRectMax();
    const bool   hv = ImGui::IsItemHovered();
    const bool   own_tint = (tint != GizmoCol::TextDim);
    if (hv || always_lit)
        dl->AddRectFilled(a, b, danger      ? gizmo_fade(GizmoCol::Warn, hv ? 0.22f : 0.10f)
                              : own_tint    ? gizmo_fade(tint, hv ? 0.30f : 0.14f)
                                            : gizmo_fade(GizmoCol::Accent, 0.28f), 4.f);
    draw_glyph(dl, ImVec2(a.x + size * 0.18f, a.y + size * 0.18f), size * 0.64f, g,
               own_tint ? gizmo_fade(tint, hv ? 1.0f : 0.85f)
                        : (hv ? (danger ? gizmo_col_u32(GizmoCol::Warn) : gizmo_col_u32(GizmoCol::AccentBright))
                              : gizmo_fade(GizmoCol::TextDim, 0.9f)));
    if (hv && tip != nullptr)
        ImGui::SetTooltip("%s", tip);
    return pressed;
}
...
void gizmo_status_chip(const char *text, ImU32 col, bool solid)
{
    const ImVec2 ts = ImGui::CalcTextSize(text);
    const float  w  = ts.x + 0.95f * gizmo_u(), h = ts.y + 0.36f * gizmo_u();
    const ImVec2 p  = ImGui::GetCursorScreenPos();
    ImDrawList  *dl = ImGui::GetWindowDrawList();
    ...
    dl->AddText(ImVec2(p.x + 0.47f * gizmo_u(), p.y + 0.18f * gizmo_u()), solid ? ink_on(col) : col, text);
...
bool gizmo_row_slider(const char *id, const char *caption, float *v, float mn, float mx,
                    const char *fmt, const char *tip)
{
    const float col = gizmo_label_col();
    ImGui::PushID(id);
    ImGui::AlignTextToFramePadding();
    ImGui::PushStyleColor(ImGuiCol_Text, gizmo_col(GizmoCol::TextDim));
    ...
    ImGui::TextUnformatted(fit_text(caption, col - 0.4f * gizmo_u()).c_str());
    ImGui::PopStyleColor();
...
bool gizmo_row_toggle(const char *id, const char *caption, bool *v, const char *tip)
{
    ...
    const float room = std::max(col - 0.4f * gizmo_u(),
                                ImGui::GetContentRegionAvail().x - box - 0.4f * gizmo_u());
    ImGui::PushID(id);
    ImGui::AlignTextToFramePadding();
    ImGui::PushStyleColor(ImGuiCol_Text, gizmo_col(GizmoCol::TextDim));
    ...
    if (ImGui::GetItemRectSize().x + 0.4f * gizmo_u() < col)
        ImGui::SameLine(col);
    else
        ImGui::SameLine(0.f, 0.4f * gizmo_u());
...
void gizmo_stat_tile(const char *id, float w, const char *caption, const char *value, ImU32 val_col,
                   float bar01, const char *tip)
{
    const float u = gizmo_u();
    const float h = 2.6f * u;
    const float pad = 0.42f * u;
    ImGui::InvisibleButton(id, ImVec2(w, h));
    ImDrawList  *dl = ImGui::GetWindowDrawList();
    const ImVec2 a  = ImGui::GetItemRectMin(), b = ImGui::GetItemRectMax();
    dl->AddRectFilled(a, b, gizmo_fade(GizmoCol::Surface, 0.65f), 5.f);
    dl->AddText(ImGui::GetFont(), 0.64f * u, ImVec2(a.x + pad, a.y + 0.22f * u), gizmo_fade(GizmoCol::TextDim, 0.95f), caption);
    dl->AddText(ImGui::GetFont(), 0.92f * u, ImVec2(a.x + pad, a.y + 0.98f * u), val_col, value);
    if (bar01 >= 0.f) {
        const float y = b.y - 0.24f * u;
        dl->AddLine(ImVec2(a.x + pad, y), ImVec2(b.x - pad, y), gizmo_fade(GizmoCol::SurfaceHi, 0.9f), 2.f);
...
void gizmo_warn_row(const char *id, const char *text, const char *why, bool amber = true)
{
    const ImU32  col   = amber ? gizmo_col_u32(GizmoCol::Warn) : gizmo_col_u32(GizmoCol::TextDim);
    const float  u     = gizmo_u();
    const float  avail = ImGui::GetContentRegionAvail().x;
...
void GLGizmoSupportZones::render_elevation()
{
    ...
    ImDrawList *dl = ImGui::GetWindowDrawList();
    gizmo_draw_canvas(dl, p0, w, h);
    dl->PushClipRect(ImVec2(p0.x + 1.f, p0.y + 1.f), ImVec2(p0.x + w - 1.f, p0.y + h - 1.f), true);

    const ImU32 c_bed  = gizmo_col_u32(GizmoCol::GridMajor);
    const ImU32 c_part = gizmo_fade(GizmoCol::SurfaceHi, 0.85f);
    ...
    if (! m_has_target) {
        ...
        hatch_rect(dl, ImVec2(cx - 54.f, top - 16.f), ImVec2(cx + 54.f, top), gizmo_fade(GizmoCol::Grid, 0.9f), 6.f);
        dl->AddLine(ImVec2(cx - 54.f, top), ImVec2(cx + 54.f, top), gizmo_fade(GizmoCol::AccentBright, 0.85f), 2.2f);
        dashed_line(dl, ImVec2(cx - 16.f, top), ImVec2(cx + 30.f, bed), gizmo_fade(GizmoCol::Accent, 0.55f), 1.6f);
        dashed_line(dl, ImVec2(cx + 16.f, top), ImVec2(cx + 62.f, bed), gizmo_fade(GizmoCol::Accent, 0.55f), 1.6f);
        dl->AddLine(ImVec2(p0.x + 14.f, bed), ImVec2(p0.x + w - 14.f, bed), c_bed, 2.f);
        for (float t = p0.x + 14.f; t < p0.x + w - 14.f; t += 8.f)
            dl->AddLine(ImVec2(t, bed + 2.f), ImVec2(t - 5.f, bed + 8.f), gizmo_fade(GizmoCol::Grid, 0.9f), 1.f);

        auto step_dot = [&](const ImVec2 &c, const char *n, bool live) {
            dl->AddCircleFilled(c, 0.52f * u, live ? gizmo_col_u32(GizmoCol::Accent) : gizmo_col_u32(GizmoCol::Surface), 18);
            dl->AddCircle(c, 0.52f * u, live ? gizmo_col_u32(GizmoCol::AccentBright) : gizmo_fade(GizmoCol::TextDim, 0.6f), 18, 1.4f);
            dl->AddText(ImGui::GetFont(), 0.68f * u, ImVec2(c.x - 0.17f * u, c.y - 0.40f * u),
                        live ? gizmo_col_u32(GizmoCol::Ink) : gizmo_fade(GizmoCol::TextDim, 0.9f), n);
        };
        ...
        dl->AddText(ImVec2(p0.x + (w - ts.x) * 0.5f, p0.y + h - 1.15f * u), gizmo_fade(GizmoCol::TextDim, 0.95f), hint.c_str());
        dl->PopClipRect();
        return;
    }
    ...
    // aterrizaje que el motor no va a alcanzar, y así se ve sin leer un número.
    {
        ...
        const ImVec2 wedge[3] = { ImVec2(X(0.), Y(z_low)),
                                  ...
                                  ImVec2(X(reach_lo), Y(z_lo)) };
        dl->AddConvexPolyFilled(wedge, 3, gizmo_col_u32(GizmoCol::AccentGhost));
        dashed_line(dl, wedge[0], wedge[1], gizmo_fade(GizmoCol::Accent, 0.45f), 1.2f);
        dashed_line(dl, wedge[0], wedge[2], gizmo_fade(GizmoCol::Accent, 0.45f), 1.2f);
    }
    ...
    if (0. >= z_lo - 1e-6 && 0. <= z_hi) {
        ...
        for (float t = p0.x + 8.f; t < p0.x + w - 6.f; t += 9.f)
            dl->AddLine(ImVec2(t, yb + 2.f), ImVec2(t - 5.f, yb + 8.f), gizmo_fade(GizmoCol::Grid, 0.9f), 1.f);
    }
    ...
    // --- La pieza y el parche ------------------------------------------------
    {
        ...
        hatch_rect(dl, ImVec2(xa, y_top), ImVec2(xb, y_low), gizmo_fade(GizmoCol::Grid, 0.95f), 6.f);
        // El parche tomado: la línea verde de "esto ya está cogido", el mismo verde que lleva en 3D.
        dl->AddLine(ImVec2(xa, y_low), ImVec2(xb, y_low), gizmo_col_u32(GizmoCol::Optimal), 2.4f);
    }

    // --- El pilar ------------------------------------------------------------
    const bool bad = landing_out_of_reach() || (m_has_landing && pillar_crosses_object()) || footprint_too_narrow();
    const ImU32 c_line = bad ? gizmo_col_u32(GizmoCol::Warn) : gizmo_col_u32(GizmoCol::AccentBright);
    const ImU32 c_fill = bad ? gizmo_fade(GizmoCol::Warn, 0.16f) : gizmo_fade(GizmoCol::Accent, 0.22f);
    if (m_has_landing) {
        ...
        if (z_knee - z_land > 1e-4) {
            ...
            gizmo_draw_node(dl, ImVec2(X(off), Y(z_knee)), 3.4f, c_line, true);
        } else {
        ...
        if (off > 0.05) {
            ...
            gizmo_draw_pill(dl, mid, lab, c_line);
        }
        ...
        if (! m_landing_on_bed) {
            ...
            hatch_rect(dl, ImVec2(X(off + foot_lo) - 12.f, Y(z_land)),
                       ImVec2(X(off + foot_hi) + 12.f, Y(z_land) + 7.f), gizmo_fade(GizmoCol::Grid, 0.95f), 6.f);
        }
        ...
        if (off > 0.05) {
            ...
            dim_line_h(dl, X(0.), X(off), p0.y + h - 0.55f * u, gizmo_fade(GizmoCol::TextDim, 0.95f), lab);
            dashed_line(dl, ImVec2(X(0.), Y(z_low)), ImVec2(X(0.), p0.y + h - 0.55f * u), gizmo_fade(GizmoCol::Grid, 1.f), 1.f, 3.f, 4.f);
        }
    } else {
        ...
        const ImVec2      ts   = ImGui::CalcTextSize(hint.c_str());
        dl->AddText(ImVec2(p0.x + (w - ts.x) * 0.5f, p0.y + h - 1.15f * u), gizmo_fade(GizmoCol::AccentBright, 0.95f), hint.c_str());
    }
    ...
    if (hovered) {
        ...
        gizmo_draw_pill(dl, ImVec2(p0.x + 8.f, p0.y + 6.f), lab, gizmo_fade(GizmoCol::TextDim, 0.8f));
    }
...
void GLGizmoSupportZones::render_zone_cards()
{
    ...
    for (int i = 0; i < int(m_zones.size()); ++ i) {
        ...
        if (sel) {
            ...
            dup_hit = gizmo_glyph_button("##dup", icon, Glyph::Copy, false, _u8L("Duplicate").c_str());
            ImGui::SetCursorScreenPos(ImVec2(icon_a.x + icon + 0.25f * u, icon_a.y));
            del_hit = gizmo_glyph_button("##del", icon, Glyph::Trash, true, _u8L("Delete").c_str());
            if (z.locked) {
                ...
                edit_hit = gizmo_glyph_button("##edit", icon, Glyph::Edit, false,
                ...
                unlock_hit = gizmo_glyph_button("##unlock", icon, Glyph::Unlock, false,
        ...
        dl->AddRectFilled(a, b, sel ? gizmo_fade(GizmoCol::AccentDim, 0.30f)
                                    : gizmo_fade(GizmoCol::Surface, hv ? 0.85f : 0.45f), 6.f);
        if (sel) {
            dl->AddRect(a, b, gizmo_fade(GizmoCol::Accent, 0.85f), 6.f, 0, 1.3f);
            dl->AddRectFilled(a, ImVec2(a.x + 3.f, b.y), gizmo_col_u32(GizmoCol::AccentBright), 2.f);
        }
        ...
        if (editing() && z.volume_idx == m_editing_volume_idx)
            dl->AddRect(ImVec2(a.x - 1.f, a.y - 1.f), ImVec2(b.x + 1.f, b.y + 1.f),
                        gizmo_col_u32(GizmoCol::Optimal), 7.f, 0, 1.6f);
        ...
        // es lo que convierte "esta zona lleva PVA" en algo que se ve desde el otro lado del panel.
        {
            ...
            const ImU32 body = z.sterile ? gizmo_col_u32(GizmoCol::Warn)
                                         : (sel ? gizmo_col_u32(GizmoCol::AccentBright) : gizmo_fade(GizmoCol::Accent, 0.9f));
            ...
            if (z.locked)
                draw_glyph(dl, ImVec2(gx + gw - 0.15f * u, gy + gh - 0.75f * u), 0.72f * u,
                           Glyph::Lock, gizmo_col_u32(GizmoCol::Optimal));
            ...
            dl->AddLine(ImVec2(gx - 1.f, gy + gh * 0.96f), ImVec2(gx + gw + 1.f, gy + gh * 0.96f),
                        gizmo_fade(GizmoCol::TextDim, 0.7f), 1.4f);
        }
        ...
        // El número de prioridad, grande, y el nombre debajo en pequeño.
        {
            ...
            dl->AddText(ImGui::GetFont(), 1.02f * u, ImVec2(tx, a.y + 0.30f * u),
                        sel ? gizmo_col_u32(GizmoCol::Ink) : gizmo_fade(GizmoCol::Ink, 0.88f), num);
            ...
            dl->AddText(ImGui::GetFont(), 0.70f * u, ImVec2(tx, a.y + 1.55f * u),
                        gizmo_fade(GizmoCol::TextDim, 0.95f), nm.c_str());
        }
        ...
        // zona que estás mirando, que es la única en la que de verdad importa.
        {
            ...
            if (z.sterile) {
                draw_glyph(dl, ImVec2(mx1 - 0.85f * u, a.y + 0.4f * u), 0.85f * u, Glyph::Warn, gizmo_col_u32(GizmoCol::Warn));
                dl->AddText(ImGui::GetFont(), 0.64f * u, ImVec2(mx0, a.y + 1.55f * u), gizmo_col_u32(GizmoCol::Warn),
                ...
                dl->AddText(ImGui::GetFont(), 0.78f * u, ImVec2(mx1 - ts.x, a.y + 0.45f * u),
                            gizmo_fade(GizmoCol::Ink, 0.85f), cnt);
                const float y = a.y + card_h - 0.8f * u;
                dl->AddLine(ImVec2(mx0, y), ImVec2(mx1, y), gizmo_fade(GizmoCol::SurfaceHi, 0.9f), 3.f);
                const float t = float(double(z.lit) / double(max_lit));
                dl->AddLine(ImVec2(mx0, y), ImVec2(mx0 + (mx1 - mx0) * std::min(1.f, t), y),
                            gizmo_col_u32(GizmoCol::Accent), 3.f);
            }
        }
...
void GLGizmoSupportZones::render_zone_materials()
{
    ...
    const float chip = 1.6f * gizmo_u(), step = chip + 5.f;
    const float avail = ImGui::GetContentRegionAvail().x;
    ...
    for (int i = 0; i <= n_fil; ++ i) {
        ...
        if (i == 0) {
            dl->AddRectFilled(a, b, gizmo_col_u32(hv ? GizmoCol::SurfaceHi : GizmoCol::Surface), 5.f);
            draw_glyph(dl, ImVec2(a.x + 4.f, a.y + 4.f), chip - 8.f, Glyph::Cube,
                       gizmo_fade(GizmoCol::TextDim, 0.95f));
        } else {
        ...
        if (on)
            dl->AddRect(ImVec2(a.x - 2.f, a.y - 2.f), ImVec2(b.x + 2.f, b.y + 2.f),
                        gizmo_col_u32(GizmoCol::AccentBright), 7.f, 0, 2.f);
        else if (hv)
            dl->AddRect(ImVec2(a.x - 1.f, a.y - 1.f), ImVec2(b.x + 1.f, b.y + 1.f),
                        gizmo_fade(GizmoCol::Ink, 0.45f), 6.f, 0, 1.2f);
        if (hv)
    ...
    if (roof > 0) {
        gizmo_warn_row("##roofwarn", _u8L("Two tools on the contact layers.").c_str(),
...
{
    const float sz = 1.9f * gizmo_u();

    if (gizmo_glyph_toggle("##vovh", sz, m_show_overhangs, Glyph::Overhang,
    ...
    if (gizmo_glyph_toggle("##vgap", sz, m_show_gaps, Glyph::GapMap,
    ...
    if (! m_zones.empty()) {
        ...
        gizmo_status_chip(t, gizmo_col_u32(GizmoCol::Accent), false);
        if (sterile > 0) {
            std::snprintf(t, sizeof(t), "%zu %s", sterile, _u8L("sterile").c_str());
            ImGui::SameLine(0.f, 5.f);
            gizmo_status_chip(t, gizmo_col_u32(GizmoCol::Warn), false);
        }
    }

    if (m_show_overhangs) {
        if (gizmo_row_slider("##ovhdeg", _u8L("overhangs").c_str(), &m_overhang_threshold_deg, 0.f, 90.f, "%.0f deg",
                           _u8L("Highlight overhangs").c_str())) {
            m_overhang_model_dirty = true;
            apply_overhang_highlight();
    ...
    if (m_show_gaps) {
        ...
        if (gizmo_row_slider("##gapstep", _u8L("map detail").c_str(), &m_gap_step_mm, 0.3f, 4.f, "%.1f mm",
                           _u8L("Red marks surface that leans past the threshold and sits inside no zone: it might need support and nothing is holding it. Green is already caught by a zone.").c_str()))
            m_parent.set_support_zone_gaps(true, overhang_normal_z_cut(), m_gap_step_mm);
    }
    ...
    if (m_c != nullptr && m_c->object_clipper() != nullptr) {
        ...
        if (gizmo_row_slider("##clp", _u8L("section").c_str(), &clp, 0.f, 1.f, "%.2f",
    ...
    if (m_show_gaps) {
        ...
        if (gizmo_row_slider("##markera", _u8L("grid marks").c_str(), &marker_alpha, 0.10f, 1.0f, "%.2f",
...
void GLGizmoSupportZones::render_issue_tray()
{
    ...
    if (m_has_target && footprint_too_narrow())
        gizmo_warn_row("##w_narrow", _u8L("Too narrow to print").c_str(),
    ...
    if (m_has_target && m_has_landing && pillar_crosses_object())
        gizmo_warn_row("##w_cross", _u8L("It goes through the part").c_str(),
                     _u8L("It goes through the part: this pillar would be printed inside the model and weld to it.").c_str());

    if (landing_off_bed())
        gizmo_warn_row("##w_offbed", _u8L("The foot hangs off the plate").c_str(),
    ...
    if (support_style_blocks_corridor()) {
        if (m_has_target && m_has_landing && landing_offset_mm() > 0.5)
            gizmo_warn_row("##w_snug", _u8L("Support style is not Snug").c_str(),
        ...
        gizmo_warn_row("##w_snugset", _u8L("Support style set to Snug for this object.").c_str(),
    ...
    if (block_tree_mode() && m_has_target && ! all_stumps().empty() && block_tree_gap_mm() <= 0.)
        gizmo_warn_row("##w_nogap", _u8L("The stump reaches the area").c_str(),
    ...
    if (landing_out_of_reach()) {
        ...
        if (block_tree_mode()) {
            gizmo_warn_row("##w_reach", _u8L("A stump is too far for this lean").c_str(),
            ...
            gizmo_warn_row("##w_reach", _u8L("Too far for this lean").c_str(),
    ...
    if (is_tree(effective_support_type()))
        gizmo_warn_row("##w_tree", _u8L("Support type is Tree").c_str(),
    ...
    if (! m_zones.empty() && is_auto(effective_support_type())) {
        gizmo_warn_row("##w_auto", _u8L("Automatic support is also on").c_str(),
    ...
    if (m_has_target && m_has_landing && ! m_preview_model.is_initialized())
        ...
        gizmo_warn_row("##w_degen", _u8L("Nothing to build there").c_str(),
...
void GLGizmoSupportZones::render_panel_body()
{
    ...
    gizmo_section(_u8L("Zones on this object").c_str());
    ...
    if (current_object() != nullptr) {
        ...
        if (gizmo_row_toggle("##onlymine", _u8L("only my zones").c_str(), &only_mine,
    ...
    gizmo_section(editing() ? _u8L("Editing a zone").c_str() : _u8L("Build a pillar").c_str());
    ...
    if (editing()) {
        ...
        for (const ZoneRow &z : m_zones)
            if (z.volume_idx == m_editing_volume_idx)
                which = z.priority + 1;
        ImDrawList *dl = ImGui::GetWindowDrawList();
        const float bh = 1.5f * gizmo_u();
        ImGui::InvisibleButton("##editbar", ImVec2(avail * 0.68f, bh));
        const ImVec2 a = ImGui::GetItemRectMin(), b = ImGui::GetItemRectMax();
        dl->AddRectFilled(a, b, gizmo_fade(GizmoCol::Optimal, 0.16f), 5.f);
        dl->AddRectFilled(a, ImVec2(a.x + 2.f, b.y), gizmo_col_u32(GizmoCol::Optimal), 1.f);
        draw_glyph(dl, ImVec2(a.x + 0.4f * gizmo_u(), a.y + (bh - 0.85f * gizmo_u()) * 0.5f), 0.85f * gizmo_u(),
                   Glyph::Edit, gizmo_col_u32(GizmoCol::Optimal));
        char t[64];
        std::snprintf(t, sizeof(t), "%s #%zu", _u8L("Editing zone").c_str(), which);
        dl->AddText(ImVec2(a.x + 1.55f * gizmo_u(), a.y + (bh - ImGui::GetTextLineHeight()) * 0.5f),
                    gizmo_col_u32(GizmoCol::Optimal), t);
        if (ImGui::IsItemHovered())
    ...
    if (m_edit_lost_surface)
        gizmo_warn_row("##w_lostsurface", _u8L("That surface is not there any more").c_str(),
    ...
    // casilla: es un MODO, y un modo activo tiene que verse desde el otro lado de la pantalla.
    {
        ...
        auto step_button = [&](const char *id, bool active, bool done, bool enabled, Glyph g,
            ...
            const ImU32 bg = ! enabled ? gizmo_fade(GizmoCol::Surface, 0.35f)
                                       : (active ? gizmo_fade(GizmoCol::Accent, hv ? 1.0f : 0.88f)
                                                 : gizmo_col_u32(hv ? GizmoCol::SurfaceHi : GizmoCol::Surface));
            dl->AddRectFilled(a, b, bg, 6.f);
            if (done && ! active)
                dl->AddRect(a, b, gizmo_fade(GizmoCol::Optimal, 0.8f), 6.f, 0, 1.3f);
            const ImU32 ink = ! enabled ? gizmo_fade(GizmoCol::TextDim, 0.45f)
                                        : (active ? gizmo_col_u32(GizmoCol::Ink)
                                                  : (done ? gizmo_col_u32(GizmoCol::Optimal) : gizmo_fade(GizmoCol::Ink, 0.85f)));
            const float gs = 1.15f * gizmo_u();
            draw_glyph(dl, ImVec2(a.x + 0.4f * gizmo_u(), a.y + (bh - gs) * 0.5f), gs, g, ink);
            dl->AddText(ImVec2(a.x + gs + 0.75f * gizmo_u(), a.y + (bh - ImGui::GetTextLineHeight()) * 0.5f), ink,
                        fit_text(label, bw - gs - 1.1f * gizmo_u()).c_str());
    ...
    if (m_target_pick_mode) {
        ...
        if (n == 0 || live == nullptr) {
            dl->AddText(ImVec2(p.x + 0.95f * gizmo_u(), p.y + 0.15f * gizmo_u()), gizmo_fade(GizmoCol::TextDim, 0.9f),
                        _u8L("Nothing under the cursor.").c_str());
        } else {
            const ImU32 col = live->past_threshold ? gizmo_col_u32(GizmoCol::AccentBright) : gizmo_col_u32(GizmoCol::Slope);
            dl->AddCircleFilled(ImVec2(p.x + 0.35f * gizmo_u(), p.y + 0.52f * gizmo_u()), 0.22f * gizmo_u(), col, 12);
            char t[96];
            if (n > 1)
                std::snprintf(t, sizeof(t), "z = %.2f mm   ·   %d / %d", live->world_pos.z(), m_candidate_idx + 1, n);
            else
                std::snprintf(t, sizeof(t), "z = %.2f mm", live->world_pos.z());
            dl->AddText(ImVec2(p.x + 0.95f * gizmo_u(), p.y + 0.15f * gizmo_u()), col, t);
            if (ImGui::IsItemHovered())
                ImGui::SetTooltip("%s\n\n%s",
                                  live->past_threshold
    ...
    if (m_has_target) {
        ...
        std::snprintf(tip, sizeof(tip), "%s\n\n%s %.0f deg  ·  %s %.2f mm %s",
                      ...
                      _u8L("wide").c_str());
        gizmo_stat_tile("##t_lean", tw, _u8L("LEAN").c_str(), v,
                      lean > 60. ? gizmo_col_u32(GizmoCol::Forbid)
                        : lean > 50. ? gizmo_col_u32(GizmoCol::Warn)
                                     : gizmo_col_u32(GizmoCol::Optimal),
        ...
        if (block_tree_mode()) {
            ...
            gizmo_stat_tile("##t_knee", tw, _u8L("STUMPS").c_str(), sv,
                          all_stumps().empty() ? gizmo_fade(GizmoCol::TextDim, 0.9f)
                                               : gizmo_col_u32(GizmoCol::AccentBright), -1.f,
            ...
            gizmo_stat_tile("##t_knee", tw, _u8L("KNEE").c_str(), "·", gizmo_fade(GizmoCol::TextDim, 0.9f), -1.f, nullptr);
        } else {
            ...
            if (offs < 0.05) {
                gizmo_stat_tile("##t_knee", tw, _u8L("KNEE").c_str(), "·", gizmo_fade(GizmoCol::TextDim, 0.9f), -1.f,
                ...
                gizmo_stat_tile("##t_knee", tw, _u8L("KNEE").c_str(), v, gizmo_col_u32(GizmoCol::AccentBright), -1.f, tip);
            } else {
                ...
                std::snprintf(v, sizeof(v), "z %.1f", z_knee);
                gizmo_stat_tile("##t_knee", tw, _u8L("KNEE").c_str(), v, gizmo_col_u32(GizmoCol::AccentBright), -1.f,
        ...
        ImGui::SameLine(0.f, 6.f);
        if (! m_has_landing) {
            gizmo_stat_tile("##t_reach", tw, _u8L("REACH").c_str(), "·", gizmo_fade(GizmoCol::TextDim, 0.9f), -1.f, nullptr);
        } else {
            ...
            gizmo_stat_tile("##t_reach", tw, _u8L("REACH").c_str(), v,
                          landing_out_of_reach() ? gizmo_col_u32(GizmoCol::Warn) : gizmo_col_u32(GizmoCol::AccentBright),
    ...
    if (m_has_target || m_target_pick_mode) {
        ...
        ImGui::PopStyleColor();
        ImGui::SameLine(gizmo_label_col());
        ...
        for (size_t i = 0; i < shapes.size(); ++ i) {
            ...
            if (gizmo_glyph_toggle(id, 1.6f * gizmo_u(), m_foot_shape == shapes[i], glyph_of(shapes[i]),
        ...
        if (gizmo_row_toggle("##paintin", _u8L("paint inside").c_str(), &m_paint_inside,
        ...
        if (m_foot_shape != FootShape::Patch) {
            ...
            if (gizmo_row_slider("##footsize", lbl.c_str(), &m_foot_size_mm, 1.f, 200.f, "%.1f mm",
            ...
            for (int i = 0; i < 2; ++ i) {
                ...
                ImGui::PushStyleColor(ImGuiCol_Button,        on ? gizmo_col(GizmoCol::Accent) : gizmo_col(GizmoCol::Surface));
                ImGui::PushStyleColor(ImGuiCol_ButtonHovered, on ? gizmo_col(GizmoCol::AccentBright) : gizmo_col(GizmoCol::SurfaceHi));
                ImGui::PushStyleColor(ImGuiCol_Text,          on ? gizmo_col(GizmoCol::Ink) : gizmo_col(GizmoCol::TextDim));
                const bool pressed = ImGui::Button((mode_lbl[i] + "##covermode").c_str(),
                                                   ImVec2(3.4f * gizmo_u(), 0.f));
        ...
        if (m_foot_shape == FootShape::Brush) {
            ...
            ImGui::PushStyleColor(ImGuiCol_Text,
                                  m_stamps.empty() ? gizmo_col(GizmoCol::TextDim) : gizmo_col(GizmoCol::AccentBright));
        ...
        if (block_tree_mode()) {
            if (gizmo_row_slider("##stumpsize", _u8L("stump").c_str(), &m_stump_size_mm, 1.f, 40.f, "%.1f mm",
            ...
            ImGui::PushStyleColor(ImGuiCol_Text,
                                  n_st == 0 ? gizmo_col(GizmoCol::TextDim) : gizmo_col(GizmoCol::AccentBright));
    ...
    if (m_has_target) {
        ...
        if (gizmo_row_slider("##shrink", _u8L("head").c_str(), &m_footprint_shrink_mm,
        ...
        if (! block_tree_mode() &&
            gizmo_row_slider("##shrinkbase", _u8L("foot").c_str(), &m_footprint_base_mm,
                           edge_lo, edge_hi, "%.1f mm", taper.c_str())) {
            m_preview_dirty = true;
            invalidate_patch();
            m_parent.set_as_dirty();
    ...
    if (m_has_target && m_has_landing) {
        ...
        std::snprintf(tip, sizeof(tip), "%s %.0f deg %s %.2f mm %s.\n%s",
                      ...
                      double(m_lean_angle_deg) > 60.
                          ...
                          : double(m_lean_angle_deg) > 50.
                              ...
                              : _u8L("Safe range.").c_str());
        if (gizmo_row_slider("##leanangle", _u8L("lean").c_str(), &m_lean_angle_deg,
        ...
        const ImVec2 p  = ImGui::GetItemRectMin();
        const ImU32 col = m_landing_locked ? gizmo_col_u32(GizmoCol::Optimal) : gizmo_col_u32(GizmoCol::AccentBright);
        draw_glyph(dl, ImVec2(p.x + 1.f, p.y + 0.1f * gizmo_u()), 0.85f * gizmo_u(), Glyph::Landing, col);
        char t[96];
        if (m_landing_on_bed)
        ...
        dl->AddText(ImVec2(p.x + 1.15f * gizmo_u(), p.y + 0.15f * gizmo_u()), col, t);
        if (ImGui::IsItemHovered())
    ...
    ImGui::Spacing();
    {
        ...
        dl->AddRectFilled(a, b, ready ? gizmo_fade(GizmoCol::Accent, hv ? 1.0f : 0.85f)
                                      : gizmo_fade(GizmoCol::Surface, 0.5f), 7.f);
        if (ready)
            dl->AddRect(a, b, gizmo_col_u32(GizmoCol::AccentBright), 7.f, 0, 1.4f);
        const ImU32 ink = ready ? gizmo_col_u32(GizmoCol::Ink) : gizmo_fade(GizmoCol::TextDim, 0.55f);
        const std::string label = edit ? _u8L("Apply changes") : _u8L("Create the pillar");
        const ImVec2      ts    = ImGui::CalcTextSize(label.c_str());
        const float gs = 1.2f * gizmo_u();
        draw_glyph(dl, ImVec2((a.x + b.x - ts.x) * 0.5f - gs - 0.5f * gizmo_u(), a.y + (bh - gs) * 0.5f), gs, Glyph::Pillar, ink);
